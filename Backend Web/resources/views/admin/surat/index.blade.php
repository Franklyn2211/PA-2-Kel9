                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    @if(isset($pengajuanSurat) && $pengajuanSurat->count() > 0)
                                        @foreach ($pengajuanSurat as $key => $item)
                                                    <tr>
                                                        <td>{{ $key + 1 }}</td>
                                                        <td>{{ $item->resident->name ?? '-' }}</td>
                                                        <td>{{ $item->template->nama ?? '-' }}</td>
                                                        <td>{{ $item->nomor_surat }}</td>
                                                        <td>{{ $item->tanggal_pengajuan ? $item->tanggal_pengajuan->format('d F Y H:i') : '-' }}
                                                        </td>
                                                        <td>
                                              @php
                                                $badgeClass = [
                                            ...
                                            @endphp
                                                        ...
                                                        <td>{{ $item->feedback ?? '-' }}</td>
                                                        <td>{{ $item->tanggal_diselesaikan ? $item->tanggal_diselesaikan->format('d F Y H:i') : '-' }}
                                                        </td>
                                                        <td>
                                                            <div class="d-flex gap-2">
                                                                ...
                                                                <form action="{{ route('admin.surat.destroy', $item->id) }}" method="POST"
